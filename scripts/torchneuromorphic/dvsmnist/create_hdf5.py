...
def dvsmnist_load_events_from_bin(file_path, max_duration=None):
...
## modify
def dvsmnist_load_events_from_txt(file_path, max_duration=None):
    data = np.loadtxt(file_path)
    return data

def dvsmnist_get_file_names(dataset_path):
    if not os.path.isdir(dataset_path):
        raise FileNotFoundError("dvsmnist Dataset not found, looked at: {}".format(dataset_path))
    ...
    print("dvsmnist: {} train samples and {} test samples per digit (max: {} train and {} test)".format(n_train, n_test, max_n_train, max_n_test))
...
def create_events_hdf5(directory, hdf5_filename):
    fns_train, fns_test = dvsmnist_get_file_names(directory)
    ...
    with h5py.File(hdf5_filename, 'w') as f:
        ...
        for file_d in tqdm(fns_train+fns_test):
            ...
            data = dvsmnist_load_events_from_txt(file_d)
            times = data[:,0]
            addrs = data[:,1:]
