...
class UCF11Dataset(NeuromorphicDataset):
    directory = 'data/UCF11/'
    ...
    def __init__(
        ...
        self.n = 0
        self.nclasses = self.num_classes = 11
        ...
        super(UCF11Dataset, self).__init__(
                root,
                transform=transform,
...
def create_datasets(
    ...
    size = [2, 240//ds, 240//ds]

    if transform_train is None:
        transform_train = Compose([
            Downsample(factor=[dt,1,4,3]),
    ...
    if target_transform_train is None:
        target_transform_train =Compose([Repeat(chunk_size_train), toOneHot(11)])   #11 class
    if target_transform_test is None:
        target_transform_test = Compose([Repeat(chunk_size_test), toOneHot(11)])

    train_ds = UCF11Dataset(root,train=True,
                                 transform = transform_train, 
                                 target_transform = target_transform_train, 
                                 chunk_size = chunk_size_train,
                                 dt = dt)

    test_ds = UCF11Dataset(root, transform = transform_test, 
